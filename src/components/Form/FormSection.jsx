...
import { Reguister } from './modales/Reguister';
import { useState } from 'react';

export function FormSection({ onClose, onSuccess }) {
            ...
            >
                ...
                <div className="p-8">
                    ...
                    <div className="flex w-full mb-6 bg-gray-100 rounded-xl p-1 relative">
                        <div 
                            className={`absolute top-1 bottom-1 w-[calc(50%-2px)] rounded-lg bg-white shadow-md transition-all duration-300 ease-out ${
                                loginIsVisible ? 'left-1' : 'left-[calc(50%+1px)]'
                            }`} 
                        />
                        <button
                            type="button"
                            onClick={() => setLoginIsVisible(true)}
                            className={`flex-1 py-2 rounded-lg text-sm font-semibold relative z-10 transition-all duration-300 ${
                                loginIsVisible ? 'text-blue-600' : 'text-gray-500 hover:text-gray-800'
                            }`}
                        >
                            Login
                        </button>
